/*Made by Richard Ledbetter 
	...
	Revised 2018 fall -Richard Ledbetter
   ...
   while(getline(in2,line)){
    ...
    while(getline(in,line)){
        ...
        if (line==""||temp)
        {
        	...
        	//DEALS WITH IN OUT FILE
        	if((u == 281)||u==297){
    			//cout<<line<<endl;
    			string flag =convert(parse(line,','));
    			//cout<<flag<<endl;
    			int flagpoint = 13;//just trust me it's bad code I know but it works
    			for (int i = 0; i < instruct.length(); ++i)
    			{
    				if (instruct[i]=='s')
    				{
    					instruct[i] = flag[flagpoint];
    					flagpoint++;
    				}
    			}
    			//cout<<instruct<<endl;
